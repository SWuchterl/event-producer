...
# DeepNTuple
# https://github.com/AlexDeMoor/DeepTuples/tree/trimmed_beyond_1B
...
ls -l
# edmConfigDump $CMSSW_BASE/DeepNTuples/DeepNtuplizer/production/DeepNtuplizer.py \
#   outputFile=mini.root \
#   maxEvents=$NEVENTS \
#   inputFiles=file:RunIII2024Summer24MiniAODv6_$NAME_$JOBINDEX.root > dumpedConfig_$NAME_$JOBINDEX.py

cmsRun -j FrameworkJobReport.xml $CMSSW_BASE/src/DeepNTuples/DeepNtuplizer/production/DeepNtuplizer.py \
  outputFile=mini \
  maxEvents=$NEVENTS \
  inputFiles=file:RunIII2024Summer24MiniAODv6_$NAME_$JOBINDEX.root
