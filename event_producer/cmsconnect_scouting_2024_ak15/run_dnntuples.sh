...
if [ "$WORKAREA" == "local" ]; then
  ...
  CMSSWDIR="/afs/cern.ch/work/s/sewuchte/private/ScoutingProd/"
  cd $CMSSWDIR/CMSSW_15_0_6/src
...
for infile in "${ADDR[@]}"; do
  echo $infile $idx
  retry cmsRun $CMSSWDIR/CMSSW_15_0_6/src/DeepNTuples/Ntupler/test/DeepNtuplizerAK15Scout.py inputFiles=${infile} ${CMSRUNARGS}
